...
@Aspect //AOP모듈을 정의하는 애노테이션
		  //특정 기능의 전/후/예외 발생시 동작해야할 공통 로직을 개발할 때 유용
@Component //String Bean으로 정의하는 애노테이션
		   //@Controller, @Repository, @Service의 부모 애노테이션
		   //Spring Bean오로 등록하려는 클래스가 Controller, Repository, Service의 성격이 아닐때 사용.
public class TimingAspect {
	 ...
	 * 여기에서 정의된 코드가 개입된다.
	 * 
	 * 원래 실행될 메소드의 전, 후에 공통 코드를  실행한다.
	 * @param pjp 원래 실행될 메소드의 클래스와 메소드의 정보
	 * @return 원래 실행될 메소드의 반환 값
	 * @throws Throwable 
	 */
	@Around("aroundTarget()")
	public Object timingAdvice(ProceedingJoinPoint pjp) throws Throwable {
		//원래 실행될 메소드의 반환값을 저장항 변수
		Object result = null;
		
		//메소드 경과 시간을 체크하기 위해 StopWatch생성.
		StopWatch stopWatch = new StopWatch();
		stopWatch.start();
		
		//위 까지가 원래 실행될 메소드의 실행 전에 수행할 코드.
		
		//여기부터 원래 실행될 매소드를 실행 시킴.
		try {
			result = pjp.proceed();
		} catch (Throwable e) {
			//원래 실행 될 메소드가 예외를 던졌을 떄 AOP가 개입할 코드를 여기에 작성.
			throw e;
		} finally {
			//여기부터는 원해 실행될 메소드가 실행 완료되었을 때 수행되는 코드
			stopWatch.stop();
